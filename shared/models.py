...
class Part(BaseModel):
    """
    Represents a part of an artifact (e.g., a text chunk, file, or data block)
    in the A2A protocol.
    ...
    encoding: Optional[str] = Field(
        None, description="Encoding if applicable (e.g., base64 for files)."
    )


class Artifact(BaseModel):
    """
    Represents an artifact, which is a collection of parts (e.g., files, text
    blocks) in the A2A protocol.
    ...
    parts: List[Part] = Field(
        default_factory=list, description="Parts that make up the artifact."
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional metadata about the artifact."
    )


class TaskHistory(BaseModel):
    """
    Represents the history of a task, including state transitions and produced
    artifacts.
    ...
    transitions: List[Dict[str, Any]] = Field(
        default_factory=list, description="State transitions with timestamps."
    )
...
class TaskStore:
    """
    In-memory store for managing tasks, their histories, and notification
    endpoints. Used by the FastAPI server to track A2A task state and delivery.
    """
    def __init__(self):
...
class DockerConfig(BaseModel):
    """
    Represents a Docker configuration (Dockerfile or docker-compose YAML) for
    analysis.
    ...
    """
    raw_text: str = Field(
        ..., description="The Dockerfile or docker-compose YAML."
    )


class SendTaskRequest(BaseModel):
    """
    Request model for sending a Docker configuration as a task via the A2A
    protocol.
...
class Task(BaseModel):
    """
    Represents an A2A task, including its state and associated Docker
    configuration.

    Attributes:
        id (str): Task identifier.
        state (str): Task state (e.g., 'submitted').
        docker_config (DockerConfig): The Docker configuration associated with the
        task.
    """
...
class DockerFixResult(BaseModel):
    ...
    """
    patched_text: str = Field(
        ..., description="The hardened Dockerfile or docker-compose YAML."
    )
    diff_json: dict = Field(
        ..., description="A structured diff between input and output."
    )
    issues_fixed: Optional[List[str]] = Field(
        default=None, description="List of security issues fixed."
    )
    issues_remaining: Optional[List[str]] = Field(
        default=None, description="List of issues not fixed."
    )
